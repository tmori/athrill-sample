 ...
 *  の責任を負わない．
 * 
 *  $Id: tHistogramAdapter.c 933 2018-03-27 17:47:44Z ertl-hiro $
 */
...
init_hist(ID histid)
{
	...
	(void) cHistogram_initialize(histid - 1);
}
...
{
	assert(1 <= histid && histid <= N_CP_cHistogram);
	(void) cHistogram_beginMeasure(histid - 1);
}
...
{
	assert(1 <= histid && histid <= N_CP_cHistogram);
	(void) cHistogram_endMeasure(histid - 1);
}
...
{
	assert(1 <= histid && histid <= N_CP_cHistogram);
	(void) cHistogram_print(histid - 1);
}
